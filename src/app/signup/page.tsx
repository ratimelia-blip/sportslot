...
export async function POST(request: Request) {
  ...
  try {
    ...
    // 2. Create club + subscription through
    ...
    if (setupError) {
      ...
      // failed so we don't leave orphan accounts.
      try {
    ...
    return NextResponse.json({
      ...
      club_id: setupData?.club_id,
      plan_id: setupData?.plan_id,
      billing: setupData?.billing,
      trial_ends_at:
    ...
    // Cleanup if the Auth user was created
    // but something unexpected failed afterward.
    if (createdUserId) {
      try {
        const supabaseUrl =
          process.env.NEXT_PUBLIC_SUPABASE_URL
